...
public class POAEvent {
    ...
    public boolean validate() throws ContextAggregatorException {
        ...
        if (getServiceInstanceId() == null || getServiceInstanceId().isEmpty()) {
            throw new ContextAggregatorException(ContextAggregatorError.INVALID_EVENT_RECEIVED, "serviceInstanceId" + missing);
        }

        // modelVersionId
        if (getModelVersionId() == null || getModelVersionId().isEmpty()) {
            throw new ContextAggregatorException(ContextAggregatorError.INVALID_EVENT_RECEIVED, "modelVersionId" + missing);
        }

        // modelInvariantId
        if (getModelInvariantId() == null || getModelInvariantId().isEmpty()) {
            throw new ContextAggregatorException(ContextAggregatorError.INVALID_EVENT_RECEIVED, "modelInvariantId" + missing);
        }
        ...
        if (getServiceType() == null || getServiceType().isEmpty()) {
            throw new ContextAggregatorException(ContextAggregatorError.INVALID_EVENT_RECEIVED, "serviceType" + missing);
        }
        ...
        if (getxTransactionId() == null || getxTransactionId().isEmpty()) {
            throw new ContextAggregatorException(ContextAggregatorError.INVALID_EVENT_RECEIVED, "xTransactionId" + missing);
        }
